...
function myFunction3() {
        ...
        swal("Sorry!", "Country Data Already Available!", "error");
}
</script>
    <style>
        .back{
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST")
 {
  ...
  if (empty($_POST["cname"]))
  ...
  else
   {
    ...
    $ccodeerr = "Country Code is requied";
  } else {
    $ccode1 =$_POST["ccode"];
    // check if name only contains letters and whitespace
  if(strlen($ccode1)>5)
  {
      $ccodeerr="Country Code Not More Than 5";
  }
    }
   ...
   <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <h2>Add New Portal By Country</h2>
        </li>
      ...
      <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" >
       ...
       <input type="text" name="cname" placeholder="Like India" value="<?php echo $cname1;?>"><br>
                 <div class="error"><?php echo "*".$ccodeerr;?></div>
                 <div>Enter Country Code</div>
          <input type="text" name="ccode" placeholder="Like in or ar ect...." value="<?php echo $ccode1;?>"><br>
                 <div>Select Country Image</div>
          ...
          <input type="submit" name="addportal" value="Add portal">
                 ...
                 <a href="addportal.php" style="color:blue;text-decoration:none;">Add Another Data</a> 
       ...
       if(isset($_POST['addportal']))
        {
           ...
           $result="select * from tbl_portal where cname='$cname' or ccode='$ccode' or cimage='$photo'";
$res=mysqli_query($con,$result);
  $num=mysqli_num_rows($res);
  $num1=mysqli_fetch_array($res);
           if($num==0)
           {
 if(!empty($cname) and !empty($ccode) and !empty($photo))
 {
           move_uploaded_file($_FILES["f1"]["tmp_name"],"../admin/icons/".$_FILES["f1"]["name"]);
           if(empty($cnameerr) and empty($ccodeerr))
           {
            mysqli_query($con,"insert into tbl_portal(cname,ccode,cimage) values('$cname','$ccode','$photo')");
